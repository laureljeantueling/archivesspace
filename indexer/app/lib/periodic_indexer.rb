...
class PeriodicIndexer < CommonIndexer
  ...
  def handle_deletes
    ...
    page = 1
    while true
      deletes = JSONModel::HTTP.get_json("/delete-feed", :modified_since => last_mtime - WINDOW_SECONDS, :page => page)
      ...
      @@record_types.each do |type|
        ...
        while true
          records = JSONModel(type).all(:page => page,
                                        ...
                                        :modified_since => @state.get_last_mtime(repository.id, type) - WINDOW_SECONDS)
