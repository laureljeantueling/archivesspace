...
shared between servers:
     ...
     cp -av /path/to/archivesspace/clustering/files/* /aspace/
...
install the supplied init script:
     ...
     chkconfig --level 3 aspace-cluster on
...
information you will need.  In particular, you will need to know:
  ...
  * The public-facing URLs for the new tenant.  We'll use
    `staff.example.com` for the staff interface, and `www.example.com`
...
database.  You can create this from the MySQL shell:
     ...
     grant all on example.* to 'example'@'%' identified by 'example123';
...
`config/config.rb`.  When you open this file you'll see two
...
Once you have edited and saved your new tenant's configuration file,
it's time to populate the database with the default tables.  You can
do this by running the `setup-database.sh` script:

     cd /aspace/archivesspace/tenants/exampletenant/archivesspace
     scripts/setup-database.sh

That's it!  The tenant is now defined.  Our final steps are to get
instances of this tenant running on our two application servers, and
then configuring the load balancer to send requests to them.
...
If you list the `exampletenant` directory now, you will see that the
`init_tenant.sh` script has created a number of symlinks.  Most of
these refer back to the `stable` version of the ArchivesSpace software
we unpacked previously, and some contain references to the `data` and
`logs` directories stored under `/aspace.local`.
...
start them up on each server:
     ...
     /etc/init.d/aspace start-tenant exampletenant
...
editor.  You will need to:
  ...
  * Insert the tenant's hostnames for each `server_name` entry.  In
    our case these are `www.example.com` for the public interface, and
...
And, finally, browse to `http://www.example.com/` to verify that Nginx
